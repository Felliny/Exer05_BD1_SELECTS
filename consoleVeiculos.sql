...
select ca.marca,
       ca.modelo,
       sum(vi.distanciaPercorrida) as total_Distancia
...
select em.nome,
       sum(vi.distanciaPercorrida) as total_distance,
       cast(avg(vi.distanciaPercorrida) as decimal(7, 2)) as mediaDist
from empresa em, carro ca, viagem vi
...
select em.nome,
       max(vi.distanciaPercorrida) as maior_dist
...
select em.nome,
       count(ca.idEmpresa) as total_carros
from empresa em, carro ca
where em.id = ca.idEmpresa
group by em.nome
having count(ca.idEmpresa) >= 3
order by total_carros desc

--Consultar Nomes das empresas que não tem carros cadastrados

select em.nome
from empresa em left join carro c on em.id = c.idEmpresa
...
select ca.marca,
       ca.modelo
from carro ca left join viagem vi on ca.id = vi.idCarro
...
select em.nome,
       ca.marca,
       ca.modelo,
       count(vi.idCarro) as quant
from viagem vi, carro ca, empresa em
where vi.idCarro = ca.id
    and em.id = ca.idEmpresa
group by ca.modelo, ca.marca, em.nome
order by em.nome asc, quant desc
...
select em.nome,
       ca.marca,
       ca.modelo,
       vi.distanciaPercorrida,
       case when (vi.distanciaPercorrida <= 1000)
        then
            cast((vi.distanciaPercorrida * 10.00) as decimal(7, 2))
        else
            cast((vi.distanciaPercorrida * 15.00) as decimal(7, 2))
        end as Valor
from empresa em, carro ca, viagem vi
where em.id = ca.idEmpresa
    and vi.idCarro = ca.id